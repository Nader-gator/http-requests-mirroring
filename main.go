...
func scrubBearer(prefixTarget, value string) string {
	...
	for key, values := range headers {
		...
		for i, value := range values {
			if key == "Authorization" {
				if strings.HasPrefix(strings.ToLower(strings.Trim(value, " \t\n")), prefixTarget) {
					scrubbedValues[i] = scrubBearer(prefixTarget, value)
				} else {
...
func watchNetwork(netit string, producer sarama.AsyncProducer, trackingChan chan struct{}) {
	...
	count := 0
	wg := sync.WaitGroup{}
	// 1024 is the default number of connections in a connection pool
	for range 1024 {
		wg.Add(1)
		go func() {
			defer wg.Done()
