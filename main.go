...
func scrubHeaderValue(value string) string {
	value, found := strings.CutPrefix(value, "Bearer")
	value = strings.Trim(value, " ")
	if len(value) > 10 {
		retVal := string(scrubApikey([]byte(
			value[:5] + "*****" + value[len(value)-5:],
		)))

		if found {
			retVal = "Bearer " + retVal
		}
		return retVal
	}
	return "Bearer *********"
...
func scrubAuthHeaders(headers http.Header) http.Header {
...
func (h *httpStream) writeRequestToKafka(req *http.Request, body []byte, timestamp time.Time) {
	...
	meta := RequestMeta{
		...
		Header:        scrubAuthHeaders(req.Header),
		Body:          finalBody,
...
func (h *httpStream) writeResponseToKafka(res *http.Response, body []byte, timestamp time.Time) {
	...
	meta := ResponseMeta{
		...
		Header:        scrubAuthHeaders(res.Header),
		Body:          finalBody,
		SrcTransport:  h.transport.Src().String(),
		DstTransport:  h.transport.Dst().String(),
		PacketTime:    timestamp,
